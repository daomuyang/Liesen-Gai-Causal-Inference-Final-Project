...
\newpage
\begin{abstract}
\noindent Under the potential-outcomes framework, I use the Ulianova public cardiovascular dataset to estimate the causal effect of physical activity on cross-sectional cardiovascular disease (CVD) prevalence. Guided by a directed acyclic graph (DAG) and the backdoor criterion, I specify an adjustment set and triangulate inverse probability weighting (IPW), g-formula, doubly robust augmented IPW (AIPW), propensity score (PS) matching and stratification, and linear probability models (LPM). I systematically assess propensity score overlap (positivity), covariate balance, BMI mediation decomposition, adjustment-set sensitivity, and E-value bounds for unmeasured confounding. After controlling for major confounders (age, sex, smoking, alcohol, cholesterol, glucose, and BMI), physical activity shows a consistent protective association: IPW-adjusted prevalence difference $=-0.044$ (95\% CI $[-0.053,\,-0.035]$), NNT $\approx 23$ on the prevalence scale; all seven identification--estimation strategies converge in direction. Protection operates mainly through a direct path not mediated by BMI (indirect share $\approx$ 5.6\%). Because the data are cross-sectional and observational, reverse causation and residual confounding cannot be ruled out; conclusions should be read as cross-sectional supportive rather than confirmatory evidence.
...
\]
If $\mathrm{ACE}<0$, intervening to move the entire population from inactive ($A=0$) to active ($A=1$) would reduce average CVD prevalence by $|\mathrm{ACE}|$. This differs from an incidence risk difference in cohort studies: cross-sectional prevalence mixes prevalent disease with current risk, so causal extrapolation requires extra caution \cite{hernan2024whatif}. It also differs from an associational comparison of currently active versus inactive individuals, which mixes causal effects with selective entry into activity.

Figure~\ref{fig:dag} shows the causal structure. Age, sex, smoking, alcohol use, cholesterol, and glucose affect both activity and CVD prevalence, forming backdoor paths $A\leftarrow L\to Y$; BMI likewise, and may biologically serve partly as mediator $A\to M\to Y$ \cite{wang2023obesity,chen2023pa}. Unmeasured factors $U$ (e.g., family history, diet, healthcare access) that affect both $A$ and $Y$ constitute residual confounding that cannot be directly tested. The primary adjustment set is
\[
...
\]
Doubly robustness means: if $\hat e(L)$ is correct, the IPW residual term corrects errors in $\hat\mu$; if $\hat\mu(a,L)$ is correct, then even if $\hat e$ is wrong, $\E[\hat\psi(a)]$ still equals $\E[Y(a)]$ \cite{robins1994estimation,shahu2023comparison}. When a single model may misspecify $e(L)$ or $\mu(a,L)$, AIPW provides additional assurance---motivating my joint reporting of IPW, g-formula, and AIPW.

The propensity score $\pi(L)=\P(A=1\mid L)$ compresses multidimensional $L$ to one dimension \cite{rosenbaum1983central}. Rosenbaum and Rubin show: if $Y(a)\indep A\mid L$, then $Y(a)\indep A\mid \pi(L)$. Matching compares $Y$ between treated and control with similar $\hat\pi(L)$; stratification divides the sample into propensity quantile groups, estimates effects within groups, then combines with weights \cite{rosenbaum1983central,chen2024iptw}. Like IPW, both aim to block backdoor paths but implement differently: matching/stratification emphasize local comparability; IPW emphasizes global reweighting. I use 1:1 nearest-neighbor matching (caliper$=0.2\times$SD($\hat\pi$)): each active individual is matched to the nearest inactive control without replacement on the treated side, while controls may be reused; 54{,}939 active individuals are matched within the caliper (99.99\% of the active group), with post-matching $|\mathrm{SMD}|<0.02$ on all seven covariates. Quintile PS stratification tests whether ACE depends on a specific weighting scheme \cite{rosenbaum1983central}. Linear probability models (LPM) regress $Y$ on $A$ controlling $L$; when $\mu(1,L)-\mu(0,L)$ is linear and exchangeability holds, LPM also identifies ACE, often close to g-formula, but lacks AIPW's double robustness \cite{hernan2024whatif}.

\textbf{Model forms and inference details.} The propensity score uses logistic regression with linear main effects for continuous covariates (age, BMI) and categorical covariates, without squared terms or interactions. The g-formula outcome model is logistic; AIPW combines logistic $\hat e(L)$ with an OLS (LPM) outcome model $\hat\mu(a,L)$. Standard errors: 500 nonparametric bootstrap replications for IPW and adjustment sensitivity; 400 and 300 bootstrap replications for AIPW and g-formula; HC1 heteroskedasticity-robust SEs for LPM; variance formulas for matched pairs and combined across-stratum variance for PS stratification. Bootstrap intervals treat the propensity score as fixed and do not propagate PS estimation uncertainty---a conventional simplification at the course-project level.

In summary, Section~3 reports crude prevalence difference, IPW, g-formula, AIPW, LPM, PS matching, and PS stratification---all targeting $\mathrm{ACE}=\E[Y(1)-Y(0)]$ (prevalence difference) under identification assumptions. Convergence across methods supports a narrative of consistent bias direction and adequate adjustment; divergence suggests model or positivity problems \cite{shahu2023comparison,hernan2024whatif}.
...
Before adjustment, the inactive group ($A=0$, $n=13{,}446$) has CVD prevalence 53.3\%; the active group ($n=54{,}946$) has 48.5\%. The crude prevalence difference is
\[
...
Pre-weighting covariate differences (full results in Section~5, Table~\ref{tab:balance_full}) show the active group has SMD of $+0.065$ for smoking and $+0.064$ for alcohol, slightly lower BMI (SMD $=-0.035$), slightly lower age (SMD $=-0.026$), and slightly higher cholesterol (SMD $=+0.022$). Smoking and alcohol increase CVD prevalence yet are more prevalent among active individuals---without controlling $L$, active-group CVD prevalence is artificially elevated, so this mechanism, in isolation, pulls the crude effect toward zero (underestimating true protection). Slightly lower BMI works in the opposite direction, pulling crude further from zero (overestimating true protection), partially offsetting this bias. Net effect: crude $|\mathrm{ACE}|$ is slightly larger than adjusted, consistent with qualitative predictions from the backdoor structure, showing crude estimates cannot serve as causal conclusions and adjustment is necessary.

\subsection{ACE Estimates from Seven Methods}

After controlling $L$, Table~\ref{tab:est} and Figure~\ref{fig:forest} summarize seven ACE estimates. Because the outcome is cross-sectional CVD prevalence, ACE below should be read as a prevalence difference---the effect of intervention on population disease proportion---not an incidence risk difference from cohort studies. By estimation principle: weighting/regression methods (IPW, AIPW, g-formula, LPM) yield $-0.0439$ to $-0.0441$; PS matching $-0.0435$ (SE$=0.0030$, narrowest); PS stratification $-0.0466$ (SE$=0.0077$, widest). Point estimates fall within a 3 percentage-point band $[-0.047,\,-0.044]$, and all 95\% CIs exclude zero---the primary empirical finding: different uses of $\hat e(L)$ and $\E[Y\mid A,L]$ do not produce divergent conclusions, and the protective effect is statistically robust.
...
Method & ACE & 95\% CI \\
\midrule
Crude prevalence difference & $-0.0476$ & [$-0.0570$, $-0.0382$] \\
...
\caption{Forest plot of seven ACE estimates (horizontal axis: prevalence difference; vertical lines: 95\% CIs; negative values indicate activity lowers CVD prevalence)}
\label{fig:forest}
\end{figure}

Using the IPW point estimate with inactive-group prevalence $p_0=0.533$ as reference, active-group prevalence is approximately $p_0+\mathrm{ACE}\approx 0.489$, giving approximate prevalence ratio (PR) $\approx 0.92$---protective direction consistent with literature, but because the outcome is CVD prevalence rather than mortality and activity is only binary self-report, magnitudes should not be numerically compared directly to Garcia et al.'s \cite{garcia2023meta} CVD death RR (0.71). For this dataset, moving 80.3\% of active individuals from the inactive counterfactual state to active would reduce population-average CVD prevalence by about 4.4 percentage points. The number needed to treat (NNT) is approximately $1/0.044\approx 23$: roughly 23 inactive individuals would need to become active to prevent one additional case of prevalent CVD (an approximate interpretation on the cross-sectional prevalence scale), with clear public health interpretability.

This prevalence difference comprises two components: the genuine protective effect of PA on CVD development and progression, and reverse causation bias (individuals with existing CVD reducing activity). The estimate should not be equated with a pure causal effect of PA on CVD incidence. Compared with Garcia et al.'s \cite{garcia2023meta} prospective meta-analysis, the protective direction is fully consistent but the magnitude is smaller---a pattern with clear methodological rationale: (i)~cross-sectional prevalence difference is subject to disease duration and survivor effects, which dilute the observed effect relative to incidence risk differences from cohort designs; (ii)~reverse causation (CVD $\to$ reduced activity), by contrast, inflates the observed protective estimate (pulls ACE away from zero)---this works counter to (i) and (iii); (iii)~binary self-reported activity without intensity or dose information introduces non-differential misclassification, biasing effect estimates toward the null. The net reduction relative to incidence-based meta-analyses is therefore driven mainly by the prevalence versus incidence scale difference and measurement error.

\subsection{Why Multiple Methods Converge}

Why do IPW (relying on $\hat e$), g-formula (relying on $\hat\mu$), AIPW (combining both), and PS matching/stratification (relying on local comparability via $\hat\pi$) yield nearly identical ACE? From an identification perspective, all target $\E[Y(1)]-\E[Y(0)]$. Here, IPW and g-formula differ by only 0.0001 ($-0.0440$ vs $-0.0439$), and AIPW nearly coincides with both---meaning propensity and outcome models do not conflict enough at the ACE level to split conclusions; severe misspecification of one typically separates IPW and g-formula \cite{shahu2023comparison}. AIPW not triggering single-model compensation means both working models mutually support---the core value of triangulation: not repeating one regression, but cross-validating the same estimand via different identification paths \cite{shahu2023comparison,hernan2024whatif}.
...
Let $\hat e(L)=\widehat{\P}(A=1\mid L)$. Figure~\ref{fig:overlap} shows densities of $\hat e$ for $A=0$ and $A=1$. With 80.3\% active, the logistic propensity model assigns high $\hat e$ to most individuals: inactive group ($n=13{,}446$) mean $\hat e$ 0.802, range $[0.742,\,0.874]$; active group ($n=54{,}946$) mean 0.804, range $[0.745,\,0.879]$. Common support is $\hat e\in[0.745,\,0.874]$; 99.99\% of individuals fall in this interval. Positivity is not empirically violated---at each $\hat e$ level, both active and inactive individuals exist, IPW weights do not systematically explode from complete lack of controls, and PS matching finds controls within the caliper.
...
From weight structure: for inactive individuals, stabilized IPW weight is $w_i=(1-\hat P(A=1))/(1-\hat e(L_i))$ with $\hat P(A=1)=0.803$. At mean $\hat e=0.802$, $w_i\approx 0.197/0.198\approx 1.0$; for active individuals, $w_i=\hat P(A=1)/\hat e(L_i)\approx 0.803/0.802\approx 1.0$. Even at upper common-support bound $\hat e=0.874$, inactive weight is about $0.197/0.126\approx 1.56$, moderate rather than explosive. Across the full sample, stabilized weights have median 1.00, 95th percentile 1.04, and maximum 1.57; the fraction with weight $>5$ is 0. Section~3 IPW is not dominated by extreme weights---positivity is not only formally satisfied but corresponds to acceptable weight distributions \cite{chen2024iptw}.
...
This pattern is consistent with high activity prevalence (80.3\%): marginally only 19.7\% are inactive, but conditionally individuals with $e(L)<1$ may still exist---Figure~\ref{fig:overlap} tests this directly. Quintile strata each contain about 13{,}678--13{,}679 individuals, with 79--83\% active and 17--21\% inactive controls (only 17.0\% inactive in the fifth stratum); each stratum retains enough controls for within-stratum prevalence differences, but the fifth stratum has the fewest inactive individuals, consistent with the widest PS stratification CI (SE$=0.0077$). Global IPW uses all 13{,}446 inactive individuals as controls, with SE (0.0044) much smaller than PS stratification. If positivity truly failed (e.g., $\hat e\to 1$ with no $A=0$ individuals), I would simultaneously see overlap disappearance, weight explosion, and PS matching failure to pair---patterns not observed here.

The relationship between positivity and Section~3 conclusions: multi-method convergence in Section~3 is obtained under the premise that IPW/PS identification is feasible within common support; this section excludes the most severe case where estimates rely purely on extrapolation from lack of controls. The next section further tests whether IPW truly balances $L$ within common support (Section~5, Table~\ref{tab:balance_full}, shows post-weighting $|\mathrm{SMD}|<0.003$), verifying backdoor blocking in data.
...
Table~\ref{tab:balance_full} and Figure~\ref{fig:love} report SMD for seven adjustment covariates. Before weighting, active and inactive groups differ systematically: smoking ($|\mathrm{SMD}|=0.065$) and alcohol (0.064) show the largest differences, with higher prevalence among active individuals---consistent with Section~3.1 confounding direction; BMI (0.035), age (0.026), cholesterol (0.022), glucose (0.020) also differ visibly; sex (0.014) is smallest but nonzero. These differences explain why crude and adjusted ACE differ: without weighting, backdoor paths $A\leftarrow L\to Y$ are not blocked \cite{hernan2024whatif}.
...
\]
Identification requires additional assumptions \cite{vanderweele2015explanation}: (i) no unmeasured confounders between $M$ and $Y$ (no $M\leftarrow U'\to Y$); (ii) $L\setminus M$ sufficiently controls confounding for $A$--$M$ and $A$--$Y$; (iii) \textbf{no exposure--mediator interaction} (by default no $A\times M$ interaction in the linear framework; if interaction exists, NDE/NIE decomposition shifts slightly); (iv) cross-world independence. In this study, the BMI--CVD association may still be confounded by diet, lifestyle, and other $U'$, so NIE and the 5.6\% mediated share are \textbf{less credible than the total effect} and should be read as exploratory. A single cross-sectional BMI measurement cannot capture dynamic weight change---limitations to remember when interpreting NIE proportion.

\subsection{BMI Path Decomposition Results}

Table~\ref{tab:med} summarizes decomposition (500 bootstrap CIs). $a$-path coefficient $-0.183$: active individuals have mean BMI 0.18 units lower (approximately 0.18 kg/m$^2$), confirming activity associates with lower BMI. $b$-path $+0.014$: controlling $L\setminus M$, each 1-unit BMI increase raises CVD prevalence by 1.4 percentage points. Product $\mathrm{NIE}=-0.183\times 0.014\approx -0.0026$ [$-0.0040$, $-0.0013$]: activity indirectly lowers CVD prevalence through BMI by about 0.26 percentage points. $\mathrm{NDE}=-0.0441$ [$-0.0530$, $-0.0362$], about 94\% of total effect $\mathrm{TE}=-0.0468$ [$-0.0562$, $-0.0383$]; indirect path about 5.6\%.
...
Does NDE at 94\% contradict literature? Not necessarily. Chen et al.\ \cite{chen2023pa} report PA mediation through body fat, blood pressure, lipids, and other pathways among Chinese adults---I test only the BMI single path, with cross-sectional binary activity; the $a$-path is significant but limited in magnitude. Biologically, activity may also affect CVD through endothelial function, systemic inflammation, cardiorespiratory fitness, etc., not necessarily via BMI \cite{bjsports2023crf,srep2025stiffness}. Wang et al.\ \cite{wang2023obesity} find obesity indices mediate PA--hypertension associations, but the outcome here is CVD not hypertension, and BMI is only a coarse obesity proxy.

As design contrast, replacing mediator with concurrent systolic pressure $ap\_hi$ yields $\mathrm{NIE}=+0.0006$, $\mathrm{NDE}=-0.0447$, total effect $=-0.0441$---indirect path nearly zero. Cross-sectional blood pressure reflects two opposing pathways: a protective path ``activity$\to$lower blood pressure$\to$lower CVD'' and a pathological path ``existing CVD$\to$elevated blood pressure''; on a single examination these partially cancel, yielding near-zero observed indirect effect. This does not mean blood pressure is unimportant as a mediator in the real world, but that cross-sectional design cannot identify the dynamic chain activity$\to$blood pressure change$\to$CVD \cite{srep2025stiffness,hernan2024whatif}. This does not contradict significant blood pressure mediation in longitudinal cohorts, but highlights the value of longitudinal data for mechanism identification.
...
Section~3 gives overall ACE; Sections~5--6 show identification diagnostics pass and BMI mediation is a small share. This section stress-tests conclusions from three complementary angles: consistency across subgroups (age, sex, BMI tertiles, and smoking), dependence on adjustment specifications, and strength of unmeasured confounding needed to explain away estimates (E-value). Together they bound extrapolation of Section~3 causal conclusions \cite{vanderweele2017evalue,hernan2024whatif}.
...
Age-stratified IPW re-estimates $\hat e(L)$ and ACE within each stratum, testing whether protection is driven by one age group. Q4 point estimate is slightly larger ($-0.050$ vs Q2 $-0.039$), possibly because older groups have higher baseline CVD prevalence and greater absolute prevalence reduction space from activity \cite{mi2025creview}; but within-stratum sample sizes are smaller than full sample, SE about 0.009 (full-sample IPW SE 0.004), CIs are wider, insufficient to assert significant heterogeneity. This supports that full-sample Section~3 ACE is not an artifact of one age stratum but a protective direction across the life course.

\subsection{Exploratory Heterogeneity by Sex and BMI}

Beyond age, I repeat IPW by sex and BMI tertiles (excluding sex from the adjustment set within sex strata) to add public-health-relevant dimensions. Women ($n=44{,}550$) have IPW ACE $-0.035$; men ($n=23{,}842$) have $-0.059$---men show a larger point estimate in absolute value, but this is exploratory stratification without a formal sex interaction test, so I do not claim activity is more effective for men. Low/middle/high BMI tertiles yield IPW ACE of $-0.046$, $-0.043$, and $-0.044$, respectively---consistent direction and similar magnitude, echoing Section~6's small BMI-mediated share. These results describe \textbf{stratum-specific effects}, not causal effect modification identified via interaction terms.
...
Exchangeability $Y(a)\indep A\mid L$ is untestable; E-value quantifies how strong unmeasured confounding $U$ would need to be to fully explain away the observed effect \cite{vanderweele2017evalue}. Using IPW point estimate $\mathrm{ACE}=-0.0440$ and inactive-group CVD prevalence $p_0=0.533$, active-group prevalence is approximately $p_1\approx 0.489$, giving approximate prevalence ratio (PR) $\approx 0.92$ (note: E-value is computed based on the prevalence ratio; its causal interpretation is inherently weaker than E-values from incidence risk ratios in cohort designs). E-value point estimate is about 1.40; for the lower bound of the 95\% CI on ACE, E-value is about 1.46---meaning if unmeasured $U$ exists, its associations with both activity and CVD (on the ratio scale) would each need to reach about 1.4 to fully offset the observed protective effect \cite{vanderweele2017evalue}.

An E-value of 1.40 is not high: in epidemiology, smoking--CVD associations often reach RR $\approx$1.8--2.0, and family history RR $\approx$1.5--2.0 \cite{mi2025creview,hernan2024whatif}---unmeasured confounding of family-history strength could fully offset the observed effect on the prevalence-ratio scale. Family history, diet, socioeconomic status, and other $U$ may plausibly reach this magnitude of association with both activity and CVD. Because the outcome is cross-sectional CVD prevalence, the E-value here reflects unmeasured confounding strength on prevalence-scale associations; its causal force is weaker than E-values derived from incidence risk ratios in prospective cohort designs. Thus, even with Section~3--5 multi-method convergence and passing positivity and balance, I position causal conclusions as cross-sectional supportive evidence consistent across methods and diagnostics, not RCT-level confirmation. E-value closes the loop with Section~5's point that $U$ may still threaten exchangeability: testable conditions are good, untestable assumptions require caution.

Linking Section~7 with the full paper: Section~3 ACE is stable; Sections~4--5 testable identification conditions hold; Section~6 mechanisms are NDE-dominated; this section shows effects are robust across age strata and BMI tertiles, exploratory sex differences warrant caution, adjustment specifications are insensitive, but E-value indicates residual confounding cannot be ignored---providing complete basis for Section~8 conclusions and limitations.

\section{Conclusions and Limitations}

\subsection{Main Conclusions}

In observational data, this study conducts a complete causal inference pipeline around whether physical activity causally lowers CVD prevalence: identification assumptions, multi-method estimation, testable diagnostics, mechanism and sensitivity analysis. Methodologically, under DAG and backdoor guidance I specify the average causal effect estimand, sequentially applying IPW, g-formula, doubly robust AIPW, LPM, and PS matching/stratification---principally different strategies---and testing positivity and covariate balance via propensity score overlap and post-weighting balance; I further decompose BMI mediation under VanderWeele's framework, combining adjustment-set variation, subgroup analysis, and E-value to assess conclusion strength \cite{hernan2024whatif,vanderweele2015explanation,vanderweele2017evalue}.

Main findings summarize as three points. First, after controlling age, sex, smoking, alcohol, and major metabolic indicators, multiple methods agree closely in direction and magnitude on the total effect (prevalence difference $\approx -0.044$; NNT $\approx 23$), all supporting a protective association between physical activity and CVD prevalence---showing conclusions do not depend on one weighting or regression specification but have reasonable methodological robustness under stated assumptions \cite{shahu2023comparison}. Second, positivity and post-IPW covariate balance diagnostics show testable identification conditions are met in data; Section~3 effect estimates do not rest on fragile foundations of lack of controls or severe post-weighting imbalance \cite{chen2024iptw,rosenbaum1983central}. Third, mechanism analysis shows protection operates mainly through direct paths not mediated by BMI, with small indirect share via BMI; effects are protective across age strata and BMI tertiles and insensitive to reasonable adjustment changes, but E-value analysis indicates unmeasured confounding may still weaken causal interpretation \cite{vanderweele2017evalue}.
...
Several limitations require attention in interpretation and generalization. Data are cross-sectional, all variables measured at one examination time, so \textbf{reverse causation} (individuals with CVD reducing activity) cannot be excluded---this bias direction would inflate the apparent protective effect (more negative ACE); the observed 4.4 percentage-point prevalence difference may partly reflect reverse causation rather than purely prospective causal effect. Activity is binary self-report without intensity, frequency, or objective measurement, not one-to-one with guideline MVPA dose; non-differential misclassification may also attenuate effects toward the null \cite{mi2025creview}. Exchangeability is fundamentally untestable; despite passing positivity and balance, unmeasured family history, diet, socioeconomic status, etc., may constitute residual confounding, and E-value analysis shows confounding strength needed to fully explain away observed effects is not extreme \cite{vanderweele2017evalue,hernan2024whatif}. Mediation analysis relies on linear approximation and cross-sectional BMI measurement; answers about biological pathways through which activity lowers CVD remain preliminary \cite{vanderweele2015explanation}.

In sum, I position causal conclusions as cross-sectional supportive evidence consistent across methods and diagnostics, not randomized-trial confirmation. Future work may validate in longitudinal follow-up, natural experiments, or instrumental variable/MR designs; combining accelerometer activity measurement with richer covariates could improve estimand interpretability and extrapolation strength. All code, data, figures, and numerical output are public on GitHub (Appendix~\ref{app:repo}) for reproduction and extension.
...
Main tables in the text include: Table~\ref{tab:est} (seven ACE estimates), Table~\ref{tab:balance_full} (covariate balance), Table~\ref{tab:med} (BMI mediation decomposition), Table~\ref{tab:adj} (adjustment sensitivity). Main figures are listed below.
...
$\mathrm{ACE}$ & Average causal effect & $\E[Y(1)-Y(0)]$, measured as cross-sectional CVD prevalence difference \\
